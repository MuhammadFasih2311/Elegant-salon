...
if (isset($_POST['update'])) {
  ...
  $update = "UPDATE services 
             SET title='$title', description='$description', price='$price',
                 icon='$icon', image='$image_path' 
             WHERE id=$id";

  if (mysqli_query($conn, $update)) {
    header("Location: manage-services.php?msg=Service updated successfully");
    exit();
  } else {
...
<div class="container my-5 flex-grow-1">
  <div class="form-card">
    <h3 class="text-warning mb-4 text-center" data-aos="fade-down">✏️ Edit Service</h3>
    <form method="POST" enctype="multipart/form-data" class="row g-3">
      <div class="col-md-6" data-aos="fade-right">
        <label class="form-label">Title</label>
        <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($data['title']) ?>" required maxlength="30" pattern="[A-Za-z0-9\s\-\&\,\.\'\(\)\+]+"
       title="Alphabets, numbers & limited symbols (-, &, ., ', (, ), +, ,) allowed">
      </div>
      <div class="col-md-6" data-aos="fade-left">
        <label class="form-label">Icon</label>
        <input type="text" name="icon" class="form-control" value="<?= htmlspecialchars($data['icon']) ?>" required maxlength="30">
      </div>
      <div class="col-12" data-aos="fade-left"> 
        <label class="form-label">Description</label>
        <textarea name="description" class="form-control" rows="3" required maxlength="200"><?= htmlspecialchars($data['description']) ?></textarea>
      </div>
      <div class="col-md-6" data-aos="fade-right">
  ...
  <?php if (!empty($data['image'])): ?>
    <div class="mt-2">
      <img src="<?= htmlspecialchars($data['image']) ?>" alt="Service Image" width="130" height="130" class="rounded shadow-sm border">
    </div>
  <?php else: ?>
    <p class="text-muted small mt-1">No image uploaded yet.</p>
  <?php endif; ?>
</div>
